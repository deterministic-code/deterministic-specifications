...
describe("1.0.0 DatasourceTypesValidator", () => {
  ...
  test("rejects a document pinned to a different version", async () => {
    const result = await datasource.validate(VALID.replace("1.0.0", "CURRENT"));
  ...
  test("accepts unsigned integer-family field types", async () => {
    const result = await datasource.validate(`version: 1.0.0
types:
  - sample:
      fields:
        - count:
...
describe("1.0.0 other spec validators", () => {
...
describe("1.0.0 every spec validator rejects an empty document", () => {
  const validators = {
